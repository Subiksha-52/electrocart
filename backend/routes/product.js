...
router.post('/:id/reviews', async (req, res) => {
  try {
    ...
    // For real user reviews, we should get user info from authentication
    // For now, we'll require userId and userName in the request body
    const { userId, userName } = req.body;
    
    if (!userId || !userName) {
      return res.status(400).json({ error: 'User information is required for reviews' });
    }
...
router.post('/', upload.single('image'), async (req, res) => {
  try {
    const { name, category, qty, price, description, rating, features, specifications } = req.body;
...
router.put('/:id', upload.single('image'), async (req, res) => {
  try {
    const { name, category, qty, price, description, rating, features, specifications } = req.body;
    const updateData = { name, category, qty, price, description, rating, features, specifications };
